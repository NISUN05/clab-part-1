...
{
  int sum = n1 + n2;
    if (n1 > 0 && n2 > 0 && sum < 0) return true;  // positive overflow
    if (n1 < 0 && n2 < 0 && sum > 0) return true;  // negative overflow
    return false;
}
...
{
	  unsigned int bits;
    memcpy(&bits, &f, sizeof(float)); // reinterpret float bits as unsigned int
    return (bits >> 23) & 0xFF;
}
